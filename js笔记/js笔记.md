...
DOM:操作文档，比如对页面元素进行移动，大小，增添删除等操作
...
****
  ...
  <script>
        let arr = [77, 23, 12, 55, 98, 2]
        for (i = 0; i < arr.length - 1; i++) {
            for (let u = 0; u < arr.length - i - 1; u++) {
                if (arr[u] > arr[u + 1]) {
                    let num = arr[u];
                    arr[u] = arr[u + 1]
                    arr[u + 1] = num
...
<script>
  ...
  setInterval(repeat, 1000)
   ...
   //清除定时器
    let time =setInerval(repeat,1000)
    clearInterval(time)
    //再重新启动定时器
    time=setInterval(repeat,1000)
</script>
```
...
window.addEventListener('scroll', function() {
    // xxxxx
    console.log(window.scrollTop) //scrollTop用来检测滚动的像素
})
document.documentElement.scrollTop //检测页面滚动的头部距离（被卷去的头部）
~~~
...
window.addEventListener('resize', function() {
    // xxxxx
    //clientwidth 计算不包含边框，margin和滚动条的宽度
    //clientheight
})
~~~
